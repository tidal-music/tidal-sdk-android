...
/**
 * This class uses [EncryptedSharedPreferences] to securely store credentials. Pass in a
 * [SharedPreferences] instance to use a custom one, by default we inject an
 * [EncryptedSharedPreferences] instance.
 */
internal class DefaultTokensStore
@Inject
constructor(private val credentialsKey: String, private val sharedPreferences: SharedPreferences) :
    TokensStore {
    ...
    private val encryptedSharedPreferences: SharedPreferences by lazy { sharedPreferences }
    ...
    private fun loadTokens(): Tokens? =
        encryptedSharedPreferences.getString(credentialsKey, null)?.let {
            try {
                    Json.decode<Tokens>(it)
                } catch (e: Exception) {
                    LegacyCredentialsMigrator().migrateCredentials(it)
                }
                .also { tokens -> saveTokens(tokens) }
        }
    ...
    override fun eraseTokens() {
        encryptedSharedPreferences.edit().clear().apply().also { latestTokens = null }
    }
}

...
/**
 * [TimeProvider] implementation that can be used to control time flow inside a coroutine by
 * seconds. Since all classes inside `TidalAuth` use a [TimeProvider] to determine current or
 * elapsed times, even non-coroutine code will be synced with the time in this [TimeProvider]. Using
 * it with [runTest] enables us to control and measure time without having to wait the actual time
 * frame.
 *
 * @param dispatcher The [TestDispatcher] to use.
 * @param mode The [Mode] to use. Default is [Mode.SECONDS], so when running, time will tick forward
 *   1 second at a time. If you need it more fine-grained, use [Mode.MILLISECONDS].
 */

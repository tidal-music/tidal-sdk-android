...
open class ApiClient(val baseUrl: String, val client: OkHttpClient = defaultClient) {
    companion object {
        ...
        protected const val JSON_MEDIATYPE = "application/json"
        protected const val FORM_DATA_MEDIATYPE = "multipart/form-data"
        protected const val FORM_URL_ENC_MEDIATYPE = "application/x-www-form-urlencoded"
        ...
        const val BASE_URL_KEY = "com.tidal.sdk.catalog.baseUrl"
    ...
    protected inline fun <reified T> requestBody(content: T, mediaType: String?): RequestBody =
        when {
            ...
            mediaType == FORM_URL_ENC_MEDIATYPE -> {
            ...
            mediaType == null || mediaType.startsWith("application/") && mediaType.endsWith("json") ->
                if (content == null) {
                    ...
                    Serializer.moshi.adapter(T::class.java).toJson(content)
                        .toRequestBody((mediaType ?: JSON_MEDIATYPE).toMediaTypeOrNull())
                }
            mediaType == XML_MEDIA_TYPE -> throw UnsupportedOperationException("xml not currently supported.")
            mediaType == OCTET_MEDIA_TYPE && content is ByteArray ->
                content.toRequestBody(OCTET_MEDIA_TYPE.toMediaTypeOrNull())
    ...
    protected inline fun <reified T: Any?> responseBody(body: ResponseBody?, mediaType: String? = JSON_MEDIATYPE): T? {
        if(body == null) {
            return null
        }
        if (T::class.java == File::class.java) {
            ...
            val tempFile = java.nio.file.Files.createTempFile("tmp.com.tidal.sdk.catalog", null).toFile()
    ...
    protected inline fun <reified I, reified T: Any?> request(requestConfig: RequestConfig<I>): ApiResponse<T?> {
        ...
        if (requestConfig.body != null && requestConfig.headers[CONTENT_TYPE].isNullOrEmpty()) {
            requestConfig.headers[CONTENT_TYPE] = JSON_MEDIATYPE
        }
        if (requestConfig.headers[ACCEPT].isNullOrEmpty()) {
            requestConfig.headers[ACCEPT] = JSON_MEDIATYPE
        }

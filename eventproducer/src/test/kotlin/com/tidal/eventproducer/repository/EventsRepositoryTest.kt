...
class EventsRepositoryTest {
    ...
    @MockK
    private lateinit var monitoringDataSource: MonitoringLocalDataSource

    @MockK
    private lateinit var repositoryHelper: RepositoryHelper

    @MockK
    private lateinit var sqsParametersConverter: SqsRequestParametersConverter
    ...
    fun `insert event updates storing failed monitoring data when database size limit has been reached`() {
        ...
        val monitoringInfoAfterUpdate = MonitoringInfo(
            mutableMapOf(),
            mutableMapOf(),
    ...
    fun `send events to sqs invokes sendEventBatch on sqsService when user is logged in`() =
        runTest {
            ...
            val response = SendMessageBatchResponse(
    ...
    @Test
    fun `send events returns success result with payload returned by service`() =
        runTest {
            // given
            val sqsParameters = mapOf("SendMessageBatchRequestEntry.1.Id" to "1")
            val serviceResponse = SendMessageBatchResponse(
                SendMessageBatchResult(null),
            )
            every { sqsParametersConverter.getSendEventsParameters(any()) } returns sqsParameters
            every { repositoryHelper.isUserLoggedIn() } returns false
    ...
    @Test
    fun `send events returns failure result when service throws an exception`() =
        runTest {
            // given
            val sqsParameters = mapOf("SendMessageBatchRequestEntry.1.Id" to "1")
            every { sqsParametersConverter.getSendEventsParameters(any()) } returns sqsParameters
    ...
    @Test
    fun `send monitoring event invokes send event batch on sqsService when monitoring info not empty`() = runTest {
        // given
        val monitoringInfo =
    ...
    @Test
    fun `send monitoring event doesn't invoke send event batch on sqsService when monitoring info is empty`() = runTest {
        // given

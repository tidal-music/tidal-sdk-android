...
/**
 * Test that the [ApiErrorMapper] returns correct based on different inputs.
 * This is useful because we then know what inputs we can expect to work.
 */
internal class ApiErrorMapperTest {
    ...
    fun map() {
        ...
        val errorBody = mock<ResponseBody> {
            on { string() } doReturn errorBodyString
        }
        val response = mock<Response<*>> {
            on { it.errorBody() } doReturn errorBody
        }
        val original = mock<HttpException> {
            on { it.response() } doReturn response
        }

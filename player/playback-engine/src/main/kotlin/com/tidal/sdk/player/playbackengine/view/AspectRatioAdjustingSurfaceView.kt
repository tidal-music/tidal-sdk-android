...
class AspectRatioAdjustingSurfaceView : SurfaceView {
    ...
    var suggestedVideoDimen by
        Delegates.observable(SuggestedDimensions(0, 0)) { _, oldValue, newValue ->
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        val targetWidthPx =
            when (suggestedVideoDimen.allowUpscaling) {
                false -> suggestedVideoDimen.widthPx
                true ->
                    if (suggestedVideoDimen.run { widthPx >= heightPx }) {
                        Int.MAX_VALUE
                    } else {
        ...
        val calculateWidthRespectingMeasureSpecAndThenAspectRatio = {
            width =
                (targetAspectRatio * height).toInt().run {
        ...
        val calculateHeightRespectingMeasureSpecAndThenAspectRatio = {
            height =
                (width.toFloat() / targetAspectRatio).toInt().run {

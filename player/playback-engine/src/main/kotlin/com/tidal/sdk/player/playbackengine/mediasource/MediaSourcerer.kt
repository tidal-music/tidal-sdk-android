 ...
 * instance.
 *
 * It always manages at most 2 items: current and next. Note: The host application is responsible
 * for managing the rest of the play queue.
 *
...
internal class MediaSourcerer(
    ...
    var currentStreamingSession: StreamingSession? by
        Delegates.observable(null) { _, oldValue, _ -> oldValue?.reportEnd() }
        private set
    ...
    fun load(
        ...
        return playbackInfoMediaSourceFactory
            .create(currentStreamingSession, forwardingMediaProduct)
            .also { exoPlayer.setMediaSource(it) }
    }
    ...
    fun setNext(
        ...
        return if (forwardingMediaProduct != null) {
            ...
            playbackInfoMediaSourceFactory
                .create(nextStreamingSession, forwardingMediaProduct)
                .also { exoPlayer.addMediaSource(it) }
        } else {
    ...
    /**
     * This needs to be called when repeat one is enabled and we start the same item again, in order
     * to create a new streaming session that we can use for reporting.
     */
    ...
    private fun StreamingSession.reportEnd() =

...
internal class ErrorHandler(private val errorCodeFactory: ErrorCodeFactory) {
    ...
    fun getErrorCode(throwable: Throwable?, extra: ErrorCodeFactory.Extra) =
        when (throwable) {
            ...
            is SocketTimeoutException ->
                errorCodeFactory.createForTimeout(extra)

            is ConnectException, is UnknownHostException ->
                errorCodeFactory.createForNetwork(extra)
    ...
    fun getErrorEvent(throwable: Throwable?, productType: ProductType? = null): Event.Error {
        ...
        do {
            ...
            errorEvent = when (cause) {
                ...
                is HttpDataSource.InvalidResponseCodeException -> lazy {
                ...
                is HttpDataSource.HttpDataSourceException -> lazy {
                    Event.Error.Network(
                ...
                is DrmSession.DrmSessionException -> lazy {
                ...
                is Loader.UnexpectedLoaderException -> lazy {
                ...
                is BehindLiveWindowException -> lazy {
                    Event.Error.Retryable(
                ...
                is SocketTimeoutException -> lazy {
                    Event.Error.Network(
                ...
                is ConnectException, is UnknownHostException -> lazy {
                    Event.Error.Network(
                ...
                is AudioSink.InitializationException -> lazy {
                    Event.Error.NotAllowed(
    ...
    private fun handleApiError(apiError: ApiError): Event.Error {
        ...
        return when (apiError.subStatus) {
            ...
            ApiError.SubStatus.NoStreamingPrivileges -> Event.Error.NotAllowed(
                errorCode,
                apiError,
            )

            ApiError.SubStatus.UserClientNotAuthorizedForOffline -> Event.Error.NotAllowed(
                errorCode,
                apiError,
            )
            ...
            ApiError.SubStatus.NoContentAvailableInProduct -> Event.Error.NotAllowed(
                errorCode,
                apiError,
            )

            ApiError.SubStatus.NoContentMatchingRequest -> Event.Error.NotAllowed(
                errorCode,
                apiError,
            )
            ...
            ApiError.SubStatus.NoContentMatchingClient -> Event.Error.NotAllowed(
                errorCode,
                apiError,
            )
            ...
            is ApiError.SubStatus.Unknown ->
                if (apiError.status in responseCode5xxRange ||

...
class Base64JwtDecoder(private val base64Codec: Base64Codec) {
    ...
    fun getClaims(token: String) = token
        .split(PAYLOAD_SPLIT_DELIMITER)
        .getOrNull(1)?.getClaimsFromPayload()

    private fun String.getClaimsFromPayload(): JsonObject {
        val decodedPayload = base64Codec.decode(toByteArray(CHARSET), Base64.URL_SAFE)
            .toString(CHARSET)
